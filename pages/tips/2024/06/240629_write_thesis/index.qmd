...
[【Typst】Quartoで論文を書く【PDF】](../../../2025/10/251021_quarto_typst/index.qmd)
:::
...
[R MarkdownでPDF出力【LaTeX】](../../03/240329_rmarkdown_pdf/index.html)
:::

::: {.callout-tip appearance="minimal"}
[【Quarto】チャンクオプションまとめてみた【R Markdown】](../../05/240509_chunk_option/index.html)
:::
...
::: {.callout-tip appearance="minimal"}
[【R】プロジェクトを使う](../../05/240515_rproj/index.html)
:::
...
コードチャンクを使うときは、上記のように記述します。コードは YAML ヘッダーで `echo: false` としているので、出力されません。結果は出力されます^[結果も隠したいときは、代わりに `include: false` を指定します。チャンクオプションについては[こちら](../../05/240509_chunk_option/index.html)]。
...
5. `ggsave`
    ...
    - `ggsave` でプロットを保存しています。ここでは `iris_plot.pdf` という名前で保存しています。"iris_plot.pdf" の部分は任意の名前で構いません。また、パスを指定して保存することももちろんできます。`here` を使っている場合は、`here("output/iris_plot.pdf")` などとして保存することができます^[`here` については[こちら](../../05/240515_rproj/index.html)]。
