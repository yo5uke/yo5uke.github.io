...
{{< linkcard url="/pages/tips/2025/09/250919_duckdb/index.qmd" title="【Python追加】DuckDBをRで使う【duckplyrも】" no-image=true >}}
...
VSCodeの拡張機能マーケットプレイスで**marimo**を検索してインストールしてください^[2026年1月現在、marimoでヒットするのは1件のみなので間違えることはないと思いますが、marimo.ioが提供している拡張機能です。]。

![marimoのVSCode拡張機能](image/marimo_vscode_extension.png)

インストール後、`.py`ファイルを開くと右上にmarimoのアイコンが表示されるので、それをクリックするとmarimoノートブックとして開くことができます。Jupyter NotebookのVSCode拡張機能と似たようなイメージです。

![marimoのVSCode拡張機能でノートブックを開く](image/marimo_run_vscode.png)
...
**uv × marimo × DuckDB × Polars**の組み合わせは非常に強力で、以下のような利点があります。
...
DuckDBの強みは**データをメモリに読み込まずにSQLクエリを実行できる**点です。大規模なCSVやParquetファイルをPython内にすべて読み込むことなく直接クエリできるため、メモリ不足の問題を回避できます。
...
このように**ローデータからの抽出はDuckDB、集計や変形はPolars**という役割分担が可能で、しかも高速に処理できるのが非常に魅力的です。
