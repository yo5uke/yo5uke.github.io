...
Rでは列名を単にベクトルで指定しますが、Polarsではリストで指定します。Rユーザーにとっては地味に面倒ですが…。
...
Rの`filter()`では`filter(Sepal.Length > 5.0)`のように書きますが、Polarsでは`pl.col("Sepal.Length")`のように列を指定します。正直こちらもRの方が書きやすいなとか思ってしまいますが、慣れの問題でしょうか。
