...
本ページでは、合成コントロール法（SCM）とその関連手法を統一インターフェースで提供するパッケージ、`coresynth`について説明します。関数リファレンスや変更履歴など詳細は[こちら](https://yo5uke.com/coresynth/)も参考にしてください。

`coresynth`の最大の特徴は**パフォーマンス**です。計算上のボトルネックをすべてC++（RcppArmadillo）で実装しており、純Rの代替実装と比べて**かなり高速化**しています。

このパッケージでは、推定だけでなく**可視化・推論・ロバストネス確認・JSON出力**まで一連の分析フローを同じインターフェースで実行できます。たとえばConformal推論（`conformal_inference()`）を使えば、`scm`/`sdid`/`gsc`/`mc`/`si`のシャープフィットに対して置換p値と信頼区間を計算できます。
...
- 6つの推定量（`scm`/`sdid`/`gsc`/`mc`/`tasc`/`si`）を`scm_fit()`で統一的に推定
- SharpだけでなくStaggered Adoption（およびSIではマルチアーム）にも対応
- `plot()`でトレンド差・ギャップ・ウェイトを共通フォーマットで可視化
...
実務的には、`predictors = NULL`（各時点を使う）を基準にしつつ、ノイズを平滑化したい場合に期間平均の`pred()`を使い分けるのがおすすめです。
...
- SCMでドナー単位のプラセボ比較を行いたい：`mspe_ratio_pval()`
...
SCMでは、MSPE比率置換検定に加えて次の診断も利用できます。
