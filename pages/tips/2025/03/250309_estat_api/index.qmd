...
1. `.Renviron`を作成し、`E_STAT_APP_ID=123456789`のようにアプリケーションIDを保存する
2. Rで`Sys.getenv("E_STAT_APP_ID")`を使って環境変数からアプリケーションIDを取得する
3. `sprintf()`を使ってURLの中の`%s`をアプリケーションIDで置き換える

```{r}
url_base <- "https://api.e-stat.go.jp/rest/3.0/app/getStatsData?appId=%s&lang=J&statsDataId=0003445078&metaGetFlg=Y&cntGetFlg=N&explanationGetFlg=Y&annotationGetFlg=Y&sectionHeaderFlg=1&replaceSpChars=0"

appId <- Sys.getenv("E_STAT_APP_ID")

url <- sprintf(url_base, appId)
```

`sprintf()`は文字列の中に変数を埋め込む関数です。`%s`の部分が`appId`で置き換えられるようになっています。

`.Renviron`ファイルはRの環境変数を保存するためのファイルで、プロジェクトのルートディレクトリに作成してください。Gitを使っている場合は`.Renviron`を`.gitignore`に追加して、アプリケーションIDがリポジトリに含まれないようにする必要があります。今回のアプリケーションIDやAIのAPIキーなど、セキュリティ上の理由から公開したくない情報は`.Renviron`に保存して環境変数として管理することが推奨されます。
...
次に`xml_find_first()`です。これは括弧内で指定したパスと合致する最初のものを抽出してくるという関数です。例えば上の1番では、`"//TABLE_INF/STATISTICS_NAME"`に一致する部分を探して、そこに書かれている内容を引っ張ってきます。最初のもの、ということは複数ある場合はどうなるのか？と僕も最初は思ったのですが、どうやら1つしかない要素に対しても一般的に使うようです^[今回取得してきたような要素は全体を見ても1か所にしか出てきません。]。
...
e-Stat APIでは、1回のリクエストで取得できるデータ件数の上限は**デフォルトで10万件**です。取得したいデータが10万件を超える場合、レスポンスのXML内に`<NEXT_KEY>`タグが含まれ、次回取得の開始位置が示されます。その場合はリクエストURLに`&startPosition=<NEXT_KEYの値>`を追加して複数回リクエストを送る必要があります。

今回の国勢調査データは件数が少ないため問題ありませんが、大規模なデータを扱う際には注意してください。
:::
