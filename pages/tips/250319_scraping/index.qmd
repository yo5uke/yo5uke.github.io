...
ご覧のようにページ内の20個のリンクを抽出することができました。次にこれらのリンクを使ってダウンロードしていきます。
...
if (!dir.exists(here("data/shpfiles"))) {
  dir.create(here("data/shpfiles"))
}
...
for (i in seq_along(full_urls)) {
  code <- sub(".*code=([0-9]+).*", "\\1", shp_links[i])
  file_path <- here("data/shpfiles", paste0("shpfile_", code, ".zip"))
