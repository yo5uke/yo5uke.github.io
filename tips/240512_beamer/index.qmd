---
title: "【スライド】Beamer の Metropolis テーマを Reveal.js で再現したい"
date: 2024-05-12
format: html
categories:
  - Quarto
  - Reveal.js
  - R
---

おしゃれなスライドを作りたい！

# Metropolis との出会い

$\LaTeX$ の Beamer で使える Metropolis テーマがおしゃれだと思ったので使ってみたい！

とある授業で Metropolis テーマを使っている先生がいて、そのスライドがとてもおしゃれだったわけです。

普段は Reveal.js を使っているので、Metropolis テーマを Reveal.js で再現したいと思い、できる限り頑張ってみました。

![Metropolis テーマはこんな感じです（[GitHub](https://github.com/matze/mtheme)より）](metropolis.png)

# Reveal.js で再現

::: {.callout-note title="Note"}
Reveal.js で、と書いていますが、Quarto 経由で使用することを想定しています。

Quarto での使い方については、[こちら](https://quarto.org/docs/presentations/revealjs/)、Reveal.js については[こちら](https://revealjs.com/)を参照してください。
:::

[こちら](https://github.com/yo5uke/modern-revealjs/tree/main)の GitHub レポジトリに、Metropolis テーマをもとに作成した scss ファイルがあります。

私もよくわからずに作成しましたが、scss ファイルはデザインを指定しているファイル、みたいな理解で良いと思います（笑）

# 使い方

1. このレポジトリをクローン or scss ファイルをダウンロード

    - レポジトリ自体は整えてないので、scss ファイルが手に入れば大丈夫です。

2. Quarto コードがあるディレクトリに scss ファイルを配置

    - 要は `.qmd` ファイルと `.scss` ファイルが同じフォルダに入っていれば大丈夫です。

3. `.qmd` ファイルに以下のように記述

```{.markdown filename="Quarto"}
---
title: "Title"
author: "Author"
institute: "Institute"
date: "2024-05-12"
format: 
    revealjs:
        theme: 
            - default
            - metropolis.scss
        slide-number: true
        date-format: long
---
```

4. `.qmd` ファイルをレンダリング

    - R Markdown で言うところの Knit です。

# デモ

<div>

```{=html}
<iframe class="slide-deck" src="demo/"></iframe>
```

</div>

Metropolis からヘッダー部分をアレンジしています^[塗りつぶしがうまくいかなかっただけです。]。

# まとめ

Reveal.js はシンプルでデザイン性も高く、コードの実行結果もそのまま表示できるというメリットがあります。

ぜひ試しに使ってみてください。

ちなみに、Quarto では Beamer も使えますので、普通に Metropolitan 本家を使うこともできます。単に普段使っている方で使いたかったから作ったというだけです。笑

# 参考

- [Metropolis テーマ](https://github.com/matze/mtheme)