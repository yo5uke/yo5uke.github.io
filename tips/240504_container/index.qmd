...
![この Mac では2回目なので git is already installed と表示されてます…。](homebrew.png)
...
4. Windows なら Ubuntu、Mac なら ターミナル を開き、以下のコマンドを入力して実行^[ディレクトリをしていてクローンしたい場合、`cd [ディレクトリへのパス]` で移動してから実行してください。]。
...
クローンしたフォルダを見てみると、あるはずのファイルやフォルダが見当たらないかもしれません。

![ファイルが足りない…？](finder_mac_after_clone.png)
...
正直なところ、ファイルは VSCode 上で開くことになるので、この設定は隠しファイルが邪魔であればしなくてもよいと思います^[TRUE のところを FALSE にして再度実行するだけです。]。VSCode 上ではきちんと表示されますので。
:::
...
2024年5月20日：テンプレートの R バージョン、Julia バージョン、Quarto のバージョンを更新

2024年5月22日：Mac への対応と、内容の刷新