...
2024年8月29日現在、DVCの認証プロセスでエラーが出るようになっています。そのうち改善されるかもしれませんが、エラーが出る場合はデータもGitHubで管理するようにするのが良いかもしれません。データサイズの制限はあるのですが、そこまで大きなデータを使わなければ問題ないと思います。
:::
...
3.  ユーザー名とパスワードを設定
    -   インストールが終わると、Linux ディストリビューションのユーザー名とパスワードの作成を求められます[^1]。
...
[^4]: 単独研究ならここのみで OK、共同研究ならホストが行う設定。
...
正直なところ、この設定は隠しファイルが邪魔であればしなくてもよいと思います[^7]。VSCode 上ではきちんと表示されますので。
:::
...
5.  コンソールに以下を入力して実行
    ...
    -   Dockerfile でインストールすることを指示しているので、コンソールでインストールせずとも既に使用可能です[^8]。
...
1.  R のコンソールで以下を実行
    -   新しいパッケージをインストールし、コード内で使用したら、以下のコードを実行してください[^13]。
...
[^13]: インストールしただけでコード内で使用していない場合は記録できません。
...
最初は add と push をする過程でアカウントの認証が必要になりますが、表示に従って認証を進めてください[^14]。

[^14]: 私の場合認証の読み込みが終わらない問題によく直面するので、その際にはターミナルを消し、再度新しいターミナルで実行するようにしています。
