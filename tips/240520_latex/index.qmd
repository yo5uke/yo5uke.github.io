...
面倒な作業は取っ払って $\LaTeX$ を使えるようにしよう！

# はじめに

こんにちは、Windows ユーザーです。

TeX Live はインストールするのが面倒だし、使い方もよくわからないので^[実際には少し使っていたので、並程度には使えます（笑）]、別の方法でローカルでも $\LaTeX$ を使えるようにしました。
...
事前準備は少し面倒だけど、一度やってしまえば楽、みたいなイメージで使っています。
...
PowerShell を管理者権限^[アプリ一覧から右クリックすれば「詳細」→「管理者として実行」で使えます。]で起動し、以下のコマンドを実行します。
...
インストールができたら Ubuntu をアプリ一覧から起動し、ユーザー名とパスワードを設定します。これは何でも構いません。`sudo` コマンドを使う際にパスワードが必要になりますが、本当に簡単なもので大丈夫です。
...
`Ctrl + ,` で設定画面を開き、右上にある ![](image/json.png) をクリックして `settings.json` を開きます。
...
次に、画面上部の「ターミナル」→「新しいターミナル」よりターミナルを開きます。そして以下のコマンドを実行してください。
...
![](image/open-compiler-log.png)

エラーログを見ると、どのパッケージがインストールされていないかがわかります。例えば、「! LaTeX Error: File 'xltxtra.sty' not found.」と出た場合、`xltxtra` パッケージがインストールされていないことがわかります。
