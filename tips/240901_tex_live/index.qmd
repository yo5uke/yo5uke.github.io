...
開いたら、「パッケージリスト」の状態から「未インストール」を選択します。選択すると別ウィンドウが出てきますので、読み込みが完了するまで待機し、完了したら閉じてください。
...
真ん中右に「選択項目をインストール」があると思うので、チェックしたのちクリックしてください。

## Sumatra PDFのダウンロード（任意）

**推奨する方**：既定のPDFビュワーがAdobeの方

TeXファイルを一度コンパイルするとPDFビュワーで開くことになるのですが、AdobeなどではPDFを開いたまま追加の更新をすることができません。一度コンパイルして修正したい場合、PDFファイルを一度閉じて再度コンパイルする必要があります。
...
自分の環境にあった（おそらくほとんどの人にとって一番上の）`.exe`ファイルをダウンロードします。
...
開いたら左にあるバーの下部からPC\>Windows (C:)と進み、さらにユーザー\>\[自分のユーザー名\]と進みます[^1]。

[^1]: ダウンロードフォルダやビデオフォルダがあれば、そこが正解です。
...
![](image/latexmkrc.png){fig-align="center" width="60%"}

作成できたら開きます。何で開くか聞かれると思いますが、（何で開いてもいいのですが）VSCodeでよいと思います。
...
以下の設定をする場合は隠しファイルを表示する設定にする必要があります。上の拡張子を表示する設定の下に「隠しファイル」という項目がありますので、そこにチェックを入れてください。
:::
...
次のようになるはずです。というか、以下をコピペしてユーザー名のところだけ自分のに変えるのでもいいです。
...
3.  `{ }` の中に以下の内容をペースト、保存[^2]

[^2]: `{ }`は消さないでください。また、既に何か書き込まれている場合、前の内容の最後をカンマで区切り、改行したうえで貼り付けてください。
...
VSCode上で左上の「ファイル」から「フォルダーを開く」をクリックし、今作成したフォルダをクリックし、「選択」をクリックします。
...
これは`type1cmパッケージ`が足りていないことを指しています。

そしたら最初の方で開いたTeX Live Managerを開き、`latexmk`の時と同様に未インストールを選択したうえで検索窓に`type1cm`と入力します。
