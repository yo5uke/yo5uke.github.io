...
また、デフォルトだと英語設定でキャプションが Table 1 や Figure 1 と表示されてしまうので、それを日本語に直すために `language` を追加しています。他にも自動で英語出力されてしまって困る際は[こちら](https://github.com/quarto-dev/quarto-cli/blob/main/src/resources/language/_language-ja.yml)から該当するものを探し、下に追加していってください^[本当は `lang: ja` を設定するだけで万事解決のはずなのですが、パッケージのインストールがどうしてもうまくいかないので、このような場当たり的な対処になっています。]。
...
5. `msummary`
    ...
    - `estimate` で "{estimate}{stars}" と指定しています。これは推定値とその有意性を表示するためのものです。入れたパターンと入れないパターンで試してもらいたいのですが、有意性について別に脚注が入ってしまい、上で書いた脚注と合わせて2行になってしまうので、わざわざこの設定をしています。
...
ggsave("iris_plot.png", p, dpi = 500)
```

```markdown
![iris を使用したプロット](iris_plot.png){#fig-iris}
```
...
出力された PDF は[こちら](https://drive.google.com/file/d/1JiV4DzOiD-uWAxc6fnu-8tjnxUcWpmVg/view?usp=sharing)から確認してください。
...
::: {.callout-tip title="テーマについて" collapse="true"}
...
5. `ggsave`
    ...
    - `ggsave` でプロットを保存しています。ここでは `iris_plot.png` という名前で保存しています。"iris_plot.png" の部分は任意の名前で構いません。また、パスを指定して保存することももちろんできます。`here` を使っている場合は、`here("output/iris_plot.png")` などとして保存することができます^[`here` については[こちら](https://yo5uke.github.io/tips/240515_rproj/)]。
    
    - dpi は 500 に設定しています。dpi は解像度を表しており、大きいほど画質が良くなります^[デフォルトでは300です。]。

    - なぜわざわざ一旦保存するのかということですが、図内に日本語が混ざっていると、直接出力すると文字化けしてしまうことがあるためです。保存してから出力することで、文字化けを防ぐことができます。

6. マークダウン表記で図を出力

    - `![iris を使用したプロット](iris_plot.png){#fig-iris}` で図を出力しています。`iris_plot.png` の部分は保存したファイル名になります。`{#fig-iris}` はラベルを付けるためのものです。論文内で参照するときに必要です。`@fig-iris` という書き方で論文内で参照できます。

::: {.callout-tip title="パスの指定" collapse="true"}
...
この場合、`![iris を使用したプロット](iris_plot.png){#fig-iris}` としても図が表示されません。その場合は、コードファイルからの相対パスを使用しなければなりません。すなわち `![iris を使用したプロット](../output/iris_plot.png){#fig-iris}` としてください。`../` は一つ上の階層を示しています。
:::
