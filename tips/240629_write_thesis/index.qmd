...
5. `ggsave`
    ...
    - `ggsave` でプロットを保存しています。ここでは `iris_plot.png` という名前で保存しています。"iris_plot.png" の部分は任意の名前で構いません。また、パスを指定して保存することももちろんできます。`here` を使っている場合は、`here("output/iris_plot.png")` などとして保存することができます^[`here` については[こちら](../240515_rproj/index.html)]。
