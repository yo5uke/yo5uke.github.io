...
コードチャンクを使うときは、上記のように記述します。コードは YAML ヘッダーで `echo: false` としているので、出力されません。結果は出力されます^[結果も隠したいときは、代わりに `include: false` を指定します。チャンクオプションについては[こちら](https://yo5uke.github.io/tips/240509_chunk_option/)]。
...
5. `ggsave`
    ...
    - ちなみに、画像の解像度が上がるので英語論文でも1度保存するのがいいと思います。
